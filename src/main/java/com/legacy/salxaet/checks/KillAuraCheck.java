...
public class KillAuraCheck {
    ...
    public void onAttack(Player attacker, Entity target, PlayerData data) {
        ...
        checkHitDelay(attacker, data, now);
        checkRotationSnap(attacker, data);

        data.lastAttackTime = now;
    }

    /**
     * Wurst/Thunderhack tarzi KillAura genelde "bak -> hemen vur" seklinde
     * calisir: swing cooldown'u yok sayarak art arda cok hizli vurur. Vanilla
     * bir oyuncunun elle bu kadar sik vurmasi neredeyse imkansizdir.
     */
    private void checkHitDelay(Player player, PlayerData data, long now) {
        if (data.lastAttackTime == 0) return;
        int minDelay = cfg().getInt("checks.killaura.min-hit-delay-ms", 45);
        long delta = now - data.lastAttackTime;
        if (delta < minDelay) {
            int v = data.addViolation("KillAura-Delay");
            alerts.raise(player, "KillAura", "vurus araligi=" + delta + "ms", v);
            checkKick(player, "KillAura", v, cfg().getInt("checks.killaura.max-violations", 6));
        }
    }

    /**
     * Saldiridan hemen once bakisin (yaw) anormal derecede buyuk ve ani
     * degismesi -> "aim-assist ile hedefe kitlenip vurma" paterni.
     */
    private void checkRotationSnap(Player player, PlayerData data) {
        if (data.yawSamples.size() < 2) return;

        Float[] yaws = data.yawSamples.toArray(new Float[0]);
        float previous = yaws[yaws.length - 2];
        float latest = yaws[yaws.length - 1];
        float diff = MathUtils.angleDiff(previous, latest);

        double maxSnap = cfg().getDouble("checks.killaura.max-rotation-snap-degrees", 55.0);
        if (diff > maxSnap) {
            int v = data.addViolation("KillAura-Snap");
            alerts.raise(player, "KillAura", String.format("bakis sicramasi=%.1f derece", diff), v);
            checkKick(player, "KillAura", v, cfg().getInt("checks.killaura.max-violations", 6));
        }
    }
